...
import { CircleCheck, Clock, ListTodo, TrendingDown, Star } from 'lucide-react'; // Added Star
import { format, subDays, isSameDay } from 'date-fns';
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';

// A small component to show a loading state
...
export default function DashboardContent({ userData, isLoading }) {
  ...
  // Use props directly. No need for complex fallbacks as the API provides defaults.
  const { name, xp, level, totalFocusTime, totalPomodoroSessions, currentStreak, history, dailyStats } = userData;
    ...
    <div className="p-4 sm:p-6 lg:p-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <div className="text-right">
          <p className="text-sm text-gray-500">Welcome back, {name || 'User'}!</p>
        ...
        <div style={{ width: '100%', height: 300 }}>
          <ResponsiveContainer>
            <LineChart data={weeklyData}>
              ...
              <Tooltip />
              <Line type="monotone" dataKey="tasksCompleted" name="Tasks Completed" stroke="#8884d8" strokeWidth={2} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-lg">
        ...
        <ul className="space-y-4">
          {history?.slice(0, 5).map((activity) => (
            <li key={activity._id} className="flex items-center space-x-4 p-3 rounded-xl hover:bg-gray-50">
          ...
          {(!history || history.length === 0) && (
            <p className="text-gray-500 text-center">No completed tasks yet. Finish a task to see it here!</p>
          )}
        </ul>
      </div>
    </div>
